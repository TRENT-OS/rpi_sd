...
typedef struct sg_mapping_iter_s
{
...
struct mmc_command_s;
struct mmc_request_s;
...
#define MMC_DATA_WRITE		(1 << 1)
	...
	int		error;
	struct mmc_command_s	*stop;
	struct mmc_request_s	*mrq;
} mmc_data_t;
...
void mmc_request_done (mmc_host_t *mmc, mmc_request_t *mrq);
